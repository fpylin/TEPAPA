...
class ngram_pattern: public matchable_pattern, public vector<hash_value> {
	...
	virtual token_string::const_iterator find(const token_string&  ts) const { return find( ts, ts.begin() ); }
	
	virtual vector<int> find_all(const class token_string&  ts) const ;
...
class extractable_ngram_pattern: public ngram_pattern, public evaluable_pattern<double> {
	...
	extractable_ngram_pattern(const vector<hash_value>& vh): ngram_pattern(vh), evaluable_pattern<double>() { set_type_id(); tag_i = -1; }	
