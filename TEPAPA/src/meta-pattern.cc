...
bool meta_pattern::match(const token_string&  ts, const matching_range& r) const {
  if ( hv_op == ght("OR") ) {
    for(const_iterator it = begin(); it != end(); ++it) {
      const pattern* p = *it;
      const matchable_pattern* mp = dynamic_cast<const matchable_pattern*>(p) ;
      if (! mp) continue;
      if (mp->match(ts, r)) return true;
    }
    return false;
  }
